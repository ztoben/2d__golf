      ...
      {editing && (
        <div class="edit-controls">
          ...
          {Object.entries(TILE_TYPES).map(([key, value]) => (
            <button onClick={() => setResource(value)}>{key}</button>
          ))}
          <button onClick={() => setResource('player')}>Player</button>
          <button onClick={() => setResource('flag')}>Flag</button>
...
function editMouseClick(p: p5, resource: () => string) {
  const x = Math.floor(p.mouseX / TILE_SIZE)
  const y = Math.floor(p.mouseY / TILE_SIZE)

  const index = y * levelState.width + x

  if (resource() === 'player') {
    setPlayerPosition(x * TILE_SIZE, y * TILE_SIZE)
  } else if (resource() === 'flag') {
    setFlagPosition(x * TILE_SIZE, y * TILE_SIZE)
  } else {
    // splice in the new map tile
